...
// Firebase Service Account Key (Kept identical to existing one)
...
serve(async (req) => {
    try {
        ...
        const { type, title, body, site_id, user_id, data: extraData } = payload;
        ...
        if (site_id) {
            // BROADCAST: Fetch FCM tokens of all residents in this site
            const { data: profiles, error: profileError } = await supabase
                .from('profiles')
                .select('fcm_token, apartments!inner(blocks!inner(site_id))')
                .eq('apartments.blocks.site_id', site_id)
                .not('fcm_token', 'is', null);

            if (profileError) throw profileError;
            tokens = profiles.map((p: any) => p.fcm_token);
        } else if (user_id) {
            // INDIVIDUAL: Fetch FCM token for specific user
        ...
        // DEDUPLICATION: Ensure unique tokens to fix the double notification bug
        ...
        const results = await Promise.all(uniqueTokens.map((token: string) =>
            fetch(fcmUrl, {
                method: 'POST',
                headers: {
